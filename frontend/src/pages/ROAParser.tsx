...
export default function ROAParser() {
  ...
  const [roaData, setRoaData] = useState<{ [year: string]: ROAForm[] }>({});
  const [roaSummary, setRoaSummary] = useState<ROASummary[]>([]);
  const [formMatching, setFormMatching] = useState<ROAFormMatching[]>([]);
  const [logs, setLogs] = useState<string>('');
  const [taxEfficiency, setTaxEfficiency] = useState<TaxEfficiencyMetrics | null>(null);

  useEffect(() => {
    // Simulate loading ROA data
    setTimeout(() => {
      const mockData = {
        '2023': [
          {
      ...
      const mockSummary: ROASummary[] = [
        ...
        },
        {
          ...
          'SE Withholding': 0,
          'Non-SE Income': 78000,
          'Non-SE Withholding': 11500,
          'Other Income': 0,
          'Other Withholding': 0,
          'Total Income': 100000,
          'Estimated AGI': 100000,
          'Total Withholding': 11500
        }
      ];

      const mockFormMatching: ROAFormMatching[] = [
        {
          filename: 'ROA_2023_TP.pdf',
          owner: 'TP',
          ssn: '***-**-3456',
          tax_period: '2023',
          form_matches: [
      ...
      if (currentData && previousData) {
        ...
        const withholdingEfficiency = 85; // Mock calculation
        ...
        const incomeStabilityScore = 75; // Mock calculation
        const yearOverYearGrowth = ((currentData['Total Income'] - previousData['Total Income']) / previousData['Total Income']) * 100;
        const projectedTaxLiability = currentData['Total Income'] * 0.22; // Simplified projection
        ...
        if (currentData['SE Income'] > 50000) serviceOpportunities.push('Business Structure Analysis');
        ...
        if (currentData['SE Income'] > 50000) recommendations.push('Evaluate S-Corp election for tax savings');
  ...
  const renderSummaryTab = () => (
    <div className="space-y-6">
      ...
      {roaSummary.length > 0 ? (
        ...
        <EmptyState
          title="No ROA Data"
          description="No ROA data extracted. Please process a case ID first."
        />
      )}
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      {!taxEfficiency ? (
        <EmptyState
          ...
          description="Tax efficiency analysis requires processed ROA data."
        />
      ) : (
        <div className="space-y-6">
          {/* Key Metrics Grid */}
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold">{formatCurrency(roaSummary.find(s => s['Tax Year'] === '2023')?.['Total Withholding'] || 0)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-600">Shortfall/Surplus</span>
                    <span className={`font-semibold ${taxEfficiency.projectedTaxLiability > (roaSummary.find(s => s['Tax Year'] === '2023')?.['Total Withholding'] || 0) ? 'text-red-600' : 'text-green-600'}`}>
                      {formatCurrency(taxEfficiency.projectedTaxLiability - (roaSummary.find(s => s['Tax Year'] === '2023')?.['Total Withholding'] || 0))}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
  ...
  const renderJsonTab = () => (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <pre className="bg-gray-900 text-green-200 rounded p-4 text-xs overflow-x-auto">
            {JSON.stringify(roaSummary, null, 2)}
          </pre>
        </CardContent>
      </Card>
    </div>
  ...
  if (isLoading) {
    return <LoadingSpinner message="Loading ROA Parser data..." />;
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="border-b border-gray-200 pb-4">
        <h1 className="text-3xl font-bold text-gray-900">ROA Parser</h1>
        <p className="mt-2 text-gray-600">
          Record of Account parsing and analysis
        </p>
      </div>
